...
- **Narration** is useful when the viewer needs context, sequence, or interpretation that the pixels do not provide. Add it when requested or when on-screen text alone is insufficient; leave it out when the source audio or visual story is already self-explanatory. Use local Kokoro only, and place the resulting audio on the same timeline as the scene it explains.
